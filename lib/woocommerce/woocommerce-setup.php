...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'miller_body_woocommerce_breakpoint' );
/**
 ...
 */
function miller_body_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'miller_body_default_products_per_page' );
/**
 ...
 */
function miller_body_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'miller_body_woocommerce_pagination' );
/**
 ...
 */
function miller_body_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'miller-body' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'miller-body' ) );

	return $args;

}

add_action( 'after_switch_theme', 'miller_body_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function miller_body_woocommerce_image_dimensions_after_theme_setup() {
	...
	miller_body_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'miller_body_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function miller_body_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function miller_body_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'miller_body_gallery_image_thumbnail' );
/**
 ...
 */
function miller_body_gallery_image_thumbnail( $size ) {
